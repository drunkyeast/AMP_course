  ...
  # get gym GPU state tensors
    ...
    def _create_envs(self):
        ...
        for i in range(self.num_envs):
            ...
            dof_prop['effort'][:] = motor_efforts_np  # 使用切片赋值
        ...
        print("effort :", dof_prop['effort'])  

        self.dof_limits_lower = to_torch(self.dof_limits_lower, device=self.device)
        self.dof_limits_upper = to_torch(self.dof_limits_upper, device=self.device)

        self._key_body_ids = self._build_key_body_ids_tensor(env_ptr, handle)
        self._contact_bodies = ["left_hand_link","right_hand_link","right_ankle_roll_link", "left_ankle_roll_link"]
    ...
    def run(self):
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            for i in range(self.control_freq_inv):
                ...
                self.gym.set_dof_actuation_force_tensor(self.sim, gymtorch.unwrap_tensor(torques)) # 向仿真中的机器人关节施加扭矩控制
                if i == self.control_freq_inv-1:
                    self.render() 
