# 先跑起来
## 安装, 这个项目名称是AMP_course,此外还有isaacgymenvs/Isaacgym这些关键词.
```sh
# 先用conda创建python=3.8的环境
# 再用pip或者conda,依照pytorch官网给的命令,安装pytorch相关
# 1.先安装isaacgym
cd ~/dev/toolchains/isaacgym/python
pip install -e .
# 2.安装本项目AMP_course和isaacgymenvs
cd ~/dev/opensource/AMP_course
pip install -e . # 在AMP_course/setup.py中可以看到包括了isaacgymenvs
# 3.查看isaacgym和isaacgymenvs的安装路径
pip list
```
## 训练
考虑到我显卡有限, 对训练进行修改.
...
python isaacgymenvs/train.py task=HumanoidAMP headless=False num_envs=512 train.params.config.minibatch_size=8192
```
```sh
# python isaacgymenvs/train.py task=G1AMP headless=True
python isaacgymenvs/train.py task=G1AMP headless=False num_envs=256 train.params.config.minibatch_size=4096
```
继续训练
此时继续训练如果不用指定参数, num_envs就默认是4096了, 具体逻辑略. 而且我之前也没用到继续训练
```sh
# headless=True, headless,无头(无显示器)
...
python isaacgymenvs/train.py task=G1AMP headless=True    checkpoint=/home/fdd/fei/AMP_Human/AMP_course/runs/G1AMP_02-22-48-27/nn/G1AMP_02-22-48-29_1000.pth

```
## 查看
```sh
# 参数理解: headless=False, 且test=True表示切换到推理模式(inference mode). 记住就好, 两个参数搭配使用.
python isaacgymenvs/train.py  task=HumanoidAMP headless=False test=True num_envs=2 checkpoint=/home/touma/dev/opensource/AMP_course/runs/HumanoidAMP_19-18-10-48/nn/HumanoidAMP_19-18-10-53_100.pth
 
# python isaacgymenvs/train.py  task=G1AMP headless=False test=True num_envs=2 checkpoint=/home/fdd/fei/AMP_Human/AMP_course/runs/G1AMP_02-23-06-14/nn/G1AMP_02-23-06-16_3000.pth
```
也可以在train脚本中, 通过sys.argv来引入参数, 挺奇怪的, 之前unitree-rl那套代码略有不同.
